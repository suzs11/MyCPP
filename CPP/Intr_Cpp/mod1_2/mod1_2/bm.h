...
public:
	...
	vector < vector<int> > graph_node;
	vector < double> graph_weight;
	vector < double> vec;
	...
	//graph part
	int N, Q, Q_true, wf, M,TM, graph_max_degree;
	int rnod;
	...
	vector <double> weight_n;
	vector <double> graph_we;
	...
	double bp_last_diff;
	vector < vector<double> > graph_we_ij;
	vector < vector<int> > graph_edge_n;
	int edge_n;
	...
	void graph_add_edge(int i, int j, double k);

	//block model
	modbp(ZRANDOMv3 *rg_, int LARGE_DEGREE_, double bp_conv_crit_, int Q_, int wf_, int vflag_, double beta_, double averr_,bool opt_);
	void set_Q(int);
	void set_vflag(int);
	...
	bool spectral(double, int, double, int);
